...
module.exports = function () {
    ...
    // First Initialize Winson Logger
    LOG.configure({transports: [
    ...
    if (isLocal) {
        LOG.silly('App is executing locally...')
        ...
        LOG.silly('VCAP environment successfully created from local config file...')
    } else {
    ...
    if (process.env.LOG_LEVEL) {
        LOG.silly('Updating the log level to ' + process.env.LOG_LEVEL)
...
function resolveEnvironmentFromVcap(envVarsLocalJson) {
    ...
    for (var valName in envVarsLocalJson) {
        LOG.silly('Resolving environment variable ' + valName)
